...
use crate::utils::mdx::markdown::parse_markdown_source;
...
pub fn Mdx(source: String, components: Components) -> impl IntoView {
    let (_fm, html) = parse_markdown_source(&source).expect("invalid mdx");
...
impl Components {
    ...
    // * 💁 (added by me, for debug purposes)
    pub fn print_info(&self) {
        println!("Components registered: {}", self.components.len());
        for name in self.components.keys() {
            println!("  - {}", name);
        }
    }

    // * 💁 Added by me. Extend the current Components with another Components instance
    pub fn extend(&mut self, other: Components) {
        self.components.extend(other.components);
    }

    // * 💁 Added by me. Merge another Components instance into this one, consuming the other instance
    ...
    fn get(&self, name: &str) -> Option<&(dyn Fn(MdxComponentProps) -> View)> {
        self.components.get(name).map(|boxed_fn| &**boxed_fn)
    }
}
