...
pub fn Chrome(
    ...
    view! {
        <div class=move || {
            ...
            <div class="dark:text-white bg-gray-75 dark:bg-gray-600/60">
                <div class={
                    let label = label.clone();
                    move || {
                        tw_merge!(
                            "absolute mx-4 flex h-12 items-center gap-2", label.get().map(|_| "sm:flex")
                        )
                    }
                }>
                    <div class="w-3 h-3 rounded-full" style:background="#ec695e" />
                    <div class="w-3 h-3 rounded-full" style:background="#f4bf4f" />
                    <div class="w-3 h-3 rounded-full" style:background="#61c653" />
                </div>
                <div class="flex justify-center items-center h-12 font-semibold">
                    {move || label.get()}
                </div>
            </div>
                ...
                >
                    <Show when=move || is_scrollable.get()>
                        <div
                            class=scrollable_x
                                .get()
                                .then_some("w-[180vw] md:w-[75rem] lg:w-[90rem]")
                    ...
                    <Provider value=ChromeContext(scrollable_ref)>{children()}</Provider>
                    <Show when=move || is_scrollable.get()>
                        <div
                            class=scrollable_x
                                .get()
